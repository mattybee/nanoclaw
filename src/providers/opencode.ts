...
// Provider/model routing only. OpenRouter (and other) API keys must NOT
// appear here: they are `sk-…` credential VALUES, the contributed-env lane
// still refuses those, and every spawn is denied. Credentials ride OneCLI
// (`HTTPS_PROXY` + host-pattern secret); OpenCode is configured with
// `apiKey: 'placeholder'` for the gateway to overwrite on the wire.
...
registerProviderContainerConfig('opencode', (ctx) => {
  ...
  if (ctx.model?.trim()) env.OPENCODE_MODEL = ctx.model.trim();
